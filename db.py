...
class Database:
    ...
    def update_user_fields(self, user_id: int, fields: Dict[str, Any]):
        if not fields:
            return
        # ✅ ensure user exists so update won't silently fail
    ...
    def add_credits(self, user_id: int, amount: int):
        # ✅ ensure user exists (important for "credit by user id")
        self.ensure_user(user_id, None)
    ...
    def set_validity(self, user_id: int, days: int):
        # ✅ ensure user exists (important for "validity by user id")
        self.ensure_user(user_id, None)
